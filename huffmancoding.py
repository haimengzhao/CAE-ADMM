...
def huffman_encode(arr, prefix, save_dir='./', save=True):
    """
    ...
    datasize = dump(data_encoding, directory/f'{prefix}.bin', save)

    # Dump codebook (huffman tree)
    codebook_encoding = encode_huffman_tree(root, dtype)
    treesize = dump(codebook_encoding, directory/f'{prefix}_codebook.bin', save)
...
def dump(code_str, filename, save=True):
    """
